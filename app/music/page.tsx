...
import { musicPlay, musicControl, musicStatus, type MusicStatus } from "@/lib/music-api";
  ...
  return (
    <div className="flex flex-col min-h-full pb-36">
      ...
      {loadingVideos ? (
        ...
        >
          {videos.map((video) => {
            const isCurrentlyPlaying = status?.song?.url?.includes(video.id) && isPlaying;
          ...
          >
            <div className="mx-auto max-w-5xl m-3">
              <div className="rounded-2xl bg-zinc-900/95 backdrop-blur-xl border border-white/10 shadow-2xl p-3 flex flex-col gap-2">
                <ProgressBar elapsed={status.elapsed ?? 0} duration={status.song.duration} />

                <div className="flex items-center gap-3">
                  {/* Thumbnail */}
                  <div className="relative shrink-0 w-11 h-11 rounded-lg overflow-hidden">
                    <img src={status.song.thumbnail} alt={status.song.name}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-white truncate">{status.song.name}</p>
                    <p className="text-xs text-white/50 truncate">{status.song.author}</p>
                  </div>

                  {/* Controls */}
                  <div className="flex items-center gap-1 shrink-0">
                    <button onClick={() => handleControl("shuffle")}
                    ...
                    <button onClick={() => handleControl("toggle")}
                    ...
                    <button onClick={() => handleControl("skip")}
                      className="p-2 rounded-lg text-white/50 hover:text-white hover:bg-white/10 transition-colors">
                    ...
                    <button onClick={() => handleControl("loop")}
                    ...
                    <button onClick={() => setShowQueue((p) => !p)}
                      className={`p-2 rounded-lg transition-colors text-xs ${
                        showQueue ? "text-primary" : "text-white/50 hover:text-white hover:bg-white/10"
                      }`}>
                      <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                    ...
                    >
                      <div className="pt-1 border-t border-white/10">
                        <p className="text-[10px] uppercase tracking-widest text-white/30 mb-2 px-1">
                          Up next · {status.queueLength - 1} song{status.queueLength !== 2 ? "s" : ""}
                        </p>
                        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
                          {status.queue.map((entry) => (
                            <div key={entry.index} className="shrink-0 w-36">
                              ...
                              <p className="text-[10px] text-white/60 mt-1 truncate">{entry.name}</p>
                            </div>
                          ))}
                        </div>
                      </div>
